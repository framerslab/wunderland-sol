...
Tool Call Request
    ↓
Step-Up Authorization (Tier 1/2/3)
    ├─ Tier 1 (Autonomous) → Execute immediately (no prompt)
    ├─ Tier 2 (Async Review) → Execute, queue for review (no prompt)
    └─ Tier 3 (Sync HITL) → Prompt user [y/a(ccept all)/N]
    ↓
Safe Guardrails (Validation)
    ├─ Match glob patterns
    ├─ Check read/write access
    └─ If HITL already approved → auto-grant folder access
    ↓
...
The tool-calling loop uses a **unified authorization flow** that ensures at most **one** interactive prompt per tool call. Previously, a single tool call could trigger up to three separate `[y/N]` prompts:

1. The `requireApprovalForAllTools` gate (for `human-all` execution mode)
2. The `StepUpAuthorizationManager` tier check (for Tier 3 tools)
3. The `SafeGuardrails` folder access escalation (for filesystem tools)

Now these gates are merged:

- **Gate 1 + Gate 2** are unified: the `StepUpAuthorizationManager` determines the tier, and only Tier 3 tools prompt the user.
- **Gate 3** respects a `hitlApprovedThisCall` flag: if the user already approved at the authorization gate, folder access is auto-granted without a second prompt.

### Safe Navigation Tools

The following tools are always classified as **Tier 1 (autonomous)** regardless of their `hasSideEffects` flag:
...
| `web_search` / `news_search` | Public web queries |
| `discover_capabilities` | Internal capability discovery |

This means a `change_directory` call on the `balanced` tier goes from **3 prompts to 0**.
