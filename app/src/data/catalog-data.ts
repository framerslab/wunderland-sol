// ============================================================================
// Shared catalog data for skills, channels, providers, tools, and secrets.
// Imported by CatalogBrowser, ApiKeyConfigurator, and the mint wizard.
// ============================================================================
...
export const CHANNELS: CatalogChannel[] = [
...
export const TOOLS: CatalogTool[] = [
  ...
  { name: 'skills', displayName: 'Skills Registry', description: 'Discover and enable curated SKILL.md prompt modules.', category: 'tool', requiredSecrets: [], packageName: '@framers/agentos-skills' },
...
// ── Secrets Map (skill/channel ID → env var names) ─────────────────────────

export const SECRETS_MAP: Record<string, { label: string; keys: string[] }> = {
  // Skills
  ...
  // Providers
  'openai': { label: 'OpenAI', keys: ['OPENAI_API_KEY'] },
  'anthropic': { label: 'Anthropic', keys: ['ANTHROPIC_API_KEY'] },
  'bedrock': { label: 'AWS Bedrock', keys: ['AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'AWS_REGION'] },
  'gemini': { label: 'Google Gemini', keys: ['GEMINI_API_KEY'] },
  'github-copilot': { label: 'GitHub Copilot', keys: ['GITHUB_COPILOT_TOKEN'] },
  'cloudflare-ai': { label: 'Cloudflare AI', keys: ['CLOUDFLARE_ACCOUNT_ID', 'CLOUDFLARE_API_TOKEN'] },
  'minimax': { label: 'Minimax', keys: ['MINIMAX_API_KEY'] },
  'qwen': { label: 'Qwen', keys: ['QWEN_API_KEY'] },
  'moonshot': { label: 'Moonshot', keys: ['MOONSHOT_API_KEY'] },
  'xiaomi-mimo': { label: 'Xiaomi Mimo', keys: ['XIAOMI_API_KEY'] },
  'venice': { label: 'Venice', keys: ['VENICE_API_KEY'] },
...
export function formatSecretEnv(secret: string): string {
  return secret.replace(/\./g, '_').toUpperCase();
}

export const SKILL_CATEGORIES = ['all', 'information', 'developer-tools', 'communication', 'productivity', 'devops', 'media', 'security', 'creative'] as const;
export const CHANNEL_TIERS = ['all', 'P0', 'P1', 'P2', 'P3'] as const;
export const TOOL_CATEGORIES = ['all', 'tool', 'voice', 'productivity'] as const;

/**
 * Collect all unique required env var keys for a set of selected skill/channel/provider IDs.
 * Returns deduplicated entries with source labels.
 */
export function collectRequiredSecrets(
  ...
  if (selectedProvider) allItems.push(selectedProvider);
